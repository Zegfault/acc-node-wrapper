...
class ACCNodeWrapper extends EventEmitter {
  constructor () {
    ...
    this.SharedMemoryInterval2 = null
    this.SharedMemoryInterval3 = null
    ...
    this.SERVER_DISPLAYNAME = null
    this.SERVER_IP = null
    this.SERVER_PORT = null
    this.SERVER_PASS = null
    this.SERVER_COMMANDPASS = null
    this.UPDATE_INTERVAL = null

    this.Logging = false
    this.Logging2 = false
    this.ConnectionId = null
    this.lastEntrylistRequest = moment()
    this._entryListCars = []
  }
  ...
  initBroadcastSDK (SERVER_DISPLAYNAME, SERVER_IP, SERVER_PORT, SERVER_PASS, SERVER_COMMANDPASS, UPDATE_INTERVAL, Logging) {
    ...
    client.on('message', (message, udpInfo) => {
      /* ==== Handling Message ==== */
      ...
      if (this.Logging) {
        ...
        console.log(`From: ${udpInfo.address}, ${udpInfo.port}`)
  ...
  handlingMessage (message) {
    ...
    switch (messageType) {
      ...
      case constants.InboundMessageTypes.ENTRY_LIST: {
        ...
        for (let i = 0; i < carEntryCount; i++) { this._entryListCars.push(new CarInfo(reader.ReadUInt16())) }
      ...
      case constants.InboundMessageTypes.TRACK_DATA: {
        ...
        for (let i = 0; i < cameraSetCount; i++) {
          ...
          for (let j = 0; j < cameraCount; j++) { cameras.push(ReadString(reader)) }
        ...
        for (let i = 0; i < hudPagesCount; i++) { result.HUDPages.push(ReadString(reader)) }
      } break
      ...
      case constants.InboundMessageTypes.SESSION_CLOSED: {
        console.warn('received SESSION_CLOSED msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAMERA_COCKPIT_MODE: {
        console.warn('received CAMERA_COCKPIT_MODE msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.RESTARTED_SESSION: {
        console.warn('received RESTARTED_SESSION msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.SESSION_START: {
        console.warn('received SESSION_START msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.PREVIOUS_CAR_FOCUS: {
        console.warn('received PREVIOUS_CAR_FOCUS msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.PLAYER_CAR_FOCUS: {
        console.warn('received PLAYER_CAR_FOCUS msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.NEXT_CAR_FOCUS: {
        console.warn('received NEXT_CAR_FOCUS msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAMERA_CYCLE_F6_CAMERAS: {
        console.warn('received CAMERA_CYCLE_F6_CAMERAS msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAMERA_CYCLE_F3_CAMERAS: {
        console.warn('received CAMERA_CYCLE_F3_CAMERAS msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAR_TELEPORTED_TO_PIT: {
        console.warn('received CAR_TELEPORTED_TO_PIT msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAR_TELEPORTED_TO_PIT_WITH_RACE_CONTROL: {
        console.warn('received CAR_TELEPORTED_TO_PIT_WITH_RACE_CONTROL msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAR_FOCUS_SINGLE_PLAYER: {
        console.warn('received CAR_FOCUS_SINGLE_PLAYER msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAR_FOCUS_MULTI_PLAYER: {
        console.warn('received CAR_FOCUS_MULTI_PLAYER msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.CAR_RETURNED_TO_PIT: {
        console.warn('received CAR_RETURNED_TO_PIT msg -> TO IMPLEMENT')
      } break
      case constants.InboundMessageTypes.STOP_CAR: {
        console.warn('received STOP_CAR msg -> TO IMPLEMENT')
      } break
  ...
  initSharedMemory (M_PHYSICS_UPDATE_INTERVAL, M_GRAPHICS_UPDATE_INTERVAL, M_STATIC_UPDATE_INTERVAL, Logging) {
    ...
    this.SharedMemoryInterval1 = setInterval(() => {
      const mPhysicsResults = this.ReadPhysics()
      this.emit('M_PHYSICS_RESULT', mPhysicsResults)
      ...
      if (this.Logging2) {
        ...
        console.log('Info: Receiving a Message.')
        console.log(`Message: ${JSON.stringify(mPhysicsResults)}`)
    ...
    this.SharedMemoryInterval2 = setInterval(() => {
      const mGraphicsResult = this.ReadGraphics()
      this.emit('M_GRAPHICS_RESULT', mGraphicsResult)
      ...
      if (this.Logging2) {
        ...
        console.log('=== Shared Memory Start ===')
        console.log('Info: Receiving a Message.')
        console.log(`Message: ${JSON.stringify(mGraphicsResult)}`)
        console.log('=== Shared Memory End ===')
        console.log('')
      }
    ...
    this.SharedMemoryInterval3 = setInterval(() => {
      const mStaticResult = this.ReadStatic()
      this.emit('M_STATIC_RESULT', mStaticResult)
      ...
      if (this.Logging2) {
        ...
        console.log('=== Shared Memory Start ===')
        console.log('Info: Receiving a Message.')
        console.log(`Message: ${JSON.stringify(mStaticResult)}`)
        console.log('=== Shared Memory End ===')
        console.log('')
      }
  ...
  /**
     ...
     */
  ReadPhysics () {
    const filePhysicsPath = 'Local\\acpmf_physics'
    this.m_physics.createMapping(null, filePhysicsPath, this.m_physics_length)
  ...
  /**
     ...
     */
  ReadGraphics () {
    const fileGraphicsPath = 'Local\\acpmf_graphics'
    this.m_graphics.createMapping(null, fileGraphicsPath, this.m_graphics_length)
  ...
  /**
     ...
     */
  ReadStatic () {
    const fileStaticPath = 'Local\\acpmf_static'
    this.m_static.createMapping(null, fileStaticPath, this.m_static_length)
